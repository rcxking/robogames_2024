...
/*
 * These averaging buffers are used to average out the computed angular
 * velocities of each wheel.  This is to avoid encoder aliasing problems.
 *
 * Aliasing occurs when the Arduino samples the encoder readings too slowly;
 * this causes the Arduino to think the motor is moving slower than it truly
 * is.  The averaging buffer is a low-pass filter that removes quick-changes
 * in the recorded motor velocity data.
 */
const size_t AVERAGING_BUFFER_SIZE = 20;
AveragingBuffer leftVelocityAveragingBuffer(AVERAGING_BUFFER_SIZE);
AveragingBuffer rightVelocityAveragingBuffer(AVERAGING_BUFFER_SIZE);

// Current left/right average velocities (rad/s) (need a variable for Serial plotter)
double avg_left_velocity = 0.0;
double avg_right_velocity = 0.0;
...
const unsigned long COMPUTE_VEL_TIME_MS = 20; // Matches the original robomagellan_2024.ino's Arduino encoder publishing rate (50 Hz)

// Current left/right wheel velocities (rad/s)
double left_wheel_vel_rad_per_sec  = 0.0;
double right_wheel_vel_rad_per_sec = 0.0;

// Time (in ms) to compute the desired motor PWM commands
const unsigned long COMPUTE_PWM_TIME_MS = 5; // Matches the original ROS 1 rpi_motors.py node's process rate (200 Hz)

// Last time motor PWM commands were calculated
unsigned long last_pwm_time_ms = 0;
...
void loop() {
  ...
  const unsigned long vel_time_delta_ms   = current_time_ms - last_vel_time_ms;

  if (vel_time_delta_ms >= COMPUTE_VEL_TIME_MS) {
    ...
    const double time_delta_s = vel_time_delta_ms / 1000.0;
    ...
    // Add the computed wheel velocities to the averaging buffers
    leftVelocityAveragingBuffer.Insert(left_wheel_vel_rad_per_sec);
    rightVelocityAveragingBuffer.Insert(right_wheel_vel_rad_per_sec);

    avg_left_velocity = leftVelocityAveragingBuffer.GetAverage();

    // DEBUG ONLY: Display the current average velocity
    //Serial.print("Left Average Angular Velocity (rad/s): ");
    Serial.println(avg_left_velocity);

    // Update last time wheel velocities were computed
    last_vel_time_ms = current_time_ms;
  }

  // Time to compute and send motor PWM commands?
  if (current_time_ms - last_pwm_time_ms >= COMPUTE_PWM_TIME_MS) {
    /*
     * Compute and send the next desired motor velocities.  We'll be using each
     * wheel's average angular velocity rather than its instantaneous angular
     * velocity to avoid aliasing.
     */
    //Serial.println("Sending PWM commands");
    leftPID.SendPWMCommand(avg_left_velocity);
    //rightPID.SendPWMCommand(rightVelocityAveragingBuffer.GetAverage());

    // Update last time PWM commands were computed
    last_pwm_time_ms = current_time_ms;
  }
}
